#!/usr/bin/env python3
"""
main_VAE_MNIST.py — CVAE Continual Learning on MNIST
=====================================================
Usage:
  # Sweep: find optimal lambda_cl and lr
  python main_VAE_MNIST.py --runtype sweep --tasks 5-5
  python main_VAE_MNIST.py --runtype sweep --tasks 5-5 --sweep_count 30 --sweep_steps 10000

  # Train: single run with specified hyperparameters
  python main_VAE_MNIST.py --runtype train --tasks 5-5 --lambda_cl 0.1 --lr 1e-3
  python main_VAE_MNIST.py --runtype train --tasks joint --lambda_cl 0.0 --no_replay
...
def build_parser():
    parser = argparse.ArgumentParser(
        description="CVAE Continual Learning — MNIST",
...
def to_3ch(x):
    """Grayscale (B,1,H,W) -> RGB (B,3,H,W) for FID/IS/LPIPS metrics."""
...
class CVAE(nn.Module):
    ...
    Adapted for MNIST (1-channel, 28x28).
    """
    def __init__(self, latent_dim, num_classes=NUM_CLASSES,
        ...
        self.final_size = image_size - 2 * n_layers  # 28 - 2*n
...
def evaluate(model, test_loader, metrics, device, args,
    ...
    PastTasks/recon,ssim   — performance on ALL previous task classes (forgetting indicator)
    ...
    with torch.no_grad():
        for x, y in test_loader:
            ...
            if full:
                x64 = F.interpolate(to_3ch(x), size=(64, 64))
                r64 = F.interpolate(to_3ch(recon), size=(64, 64))
                seen_lpips += metrics['lpips'](r64, x64).item()
                xu8 = (to_3ch(x) * 255).byte()
                ru8 = (to_3ch(recon) * 255).clamp(0, 255).byte()
    ...
    if full:
        ...
        with torch.no_grad():
            for x, y in test_loader:
                ...
                mask = torch.tensor([t.item() in seen_set for t in y], device=device)
                if mask.sum() > 0:
                    metrics['fid'].update((to_3ch(x[mask]) * 255).byte(), real=True)
            for c in gen_classes:
                ...
                gu8 = (to_3ch(gen) * 255).clamp(0, 255).byte()
        ...
        with torch.no_grad():
            ...
            if fx.size(0) > 0:
                ...
                vis_recon = torch.zeros(1, 16, 16, device=device)
